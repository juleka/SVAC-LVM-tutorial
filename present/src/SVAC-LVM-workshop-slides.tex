...
If you are interested in the math, please refer to Schnakenberg and Fariss (2014: 7-8). 
...
We can check whether our model converged successfully by plotting the R-hat statistic. 

If the values remain below 1.1, we conclude model convergence has occurred.

If the values are above 1.1, we would rerun the model with a higher number of iterations until we reach convergence.
}

\frame{\frametitle{An R-hat Plot for the Static LVM}
...
\begin{figure}
\subfigure[AI]{\includegraphics[width=3.5cm]{../estimate/output/bp-cp-static-ai.pdf}}
\subfigure[HRW]{\includegraphics[width=3.5cm]{../estimate/output/bp-cp-static-hrw.pdf}}
\subfigure[USSD]{\includegraphics[width=3.5cm]{../estimate/output/bp-cp-static-state.pdf}}
\end{figure}
}
...
\frametitle{Static Estimates of the Latent Prevalence of Wartime Sexual Violence}
\begin{figure}\centering
...
\frametitle{Static Estimates for Our Second Worst Case}
\begin{figure}\centering
...
\frametitle{Dynamic Estimates of the Latent Prevalence of Wartime Sexual Violence}
\begin{figure}\centering
...
\frametitle{Dynamic Estimates for the DRC}
\begin{figure}\centering
...
\begin{itemize}
\item Our goal is to develop a LVM approach to measurement for Wartime Sexual Violence. 
...
\frame{\frametitle{Planned Next Steps}
...
\item Conduct case studies of select cases to better understand data-generating processes and finetune modeling approach (e.g., Kashmir, Kosovo, ?)
\end{itemize}
}
